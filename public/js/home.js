...
(function () {
    ...
    function renderCinemas(payload) {
        const cinemas = (payload && payload.cinemas)
            ? payload.cinemas
            : [];
        ...
        listEl.innerHTML = cinemas.map(function (cinema) {
            ...
            const websiteBtn = cinema.maps_url
                ? '<a href="' +
                    cinema.maps_url +
                    '" target="_blank" rel="noopener" class="btn btn-sm location-btn location-btn-primary w-100 mt-2">Visit Website</a>'
                : '';
